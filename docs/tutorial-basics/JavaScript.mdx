...
8种基本数据类型, 分别是: undefined null string number boolean symbol bigint Object
...
- `typeof` 能判断所有值类型、函数, 不可以对 `null` `objct` `array` 判断 因为都返回 `object`
- `instanceof` 能判断对象类型, 不能判断基本类型, **其内部的运行机制是判断在其原型链中是否可以找到该类型的原型**
